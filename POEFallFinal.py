...
def HEAD(f,n):
    ...
    f.write('\\title{POE Final Exam}\n')
    f.write('\\author{RH Cox}\n')
    f.write('\\date{2013 December 19}\n')
    f.write('\\maketitle\n')
...
def PAGE(f,a):
    ...
    f.write('You may show detailed work on seperate page.  Results must be shown in space provided.\n\n')
    f.write('I.  Draw mesh current loops, label with current.(Max 2 ptsX6)\n\n')
    ...
    f.write('(12,0) to [short] (12,4)\n')
    f.write('(12,8) to [short] (8,8)\n')
    f.write('(8,4) to [R=$'+r18+'\\Omega$,-*] (12,4)\n')
    f.write('(12,4) to [R=$'+r1+'\\Omega$,-*] (12,8)\n')
    f.write(';\n')    
    f.write('\\end{circuitikz}\n')
    f.write('\n\n II. Write unsimplified mesh equation for each mesh loop.(Max 3 ptX6)\n\n')
    f.write('\\newpage\n\n')    
    f.write('\n\n III. Write simplified mesh equations, with current order.(Max 4 ptsX6)\n\n')
    f.write('\\vspace{5cm}\n\n')    
    f.write('\n\n IV. Write matrix representation $R \\vec{i}=\\vec{V}$.(Max 10 points)\n\n')
    f.write('\\vspace{3cm}\n\n')    
    f.write('\n\n V. Use calculator to solve for currents. Write as $\\vec{i}=$.(Max 10  points)\n\n')
    f.write('\\vspace{3cm}\n\n')    
    f.write('VI. Write currents with proper orientation and location in original circuit.(Max 1 pts X 6)\n\n')
    f.write('VII. Calculate voltages across each resistor.  Label the voltage across each resistor.(Max 2 pts X 6)\n\n')
    f.write('\\vspace{3cm}\n\n')    
    f.write('VIII. Calculate voltages across each resistor.  Label the voltage across each resistor.(Max 1 pts X 9)\n\n')
    f.write('IX. Show the sum of voltages around each mesh loop is 0.(Max 1 pts X 6)\n\n')
