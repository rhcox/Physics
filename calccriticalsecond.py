...
def HEAD(f):
	...
	f.write('\\title{Calculus Critical Values-Concavity}\n')
...
def PAGE(f,a):
    ...
    f.write('\\item Calculate the concavity for each function, Indicate on the number line.')
    f.write('\\item Write increasing and decreasing intervals.')
    f.write('\\item Try not use a calculator.')
    ...
    a=str(x)
    b=str(int((x*x)/3))
    c=str(int(random.random()*10)+1)
    f.write('\\item $f(x)=x^3+'+a+'x^2+'+b+'x+'+c+'$\n')
    x=int(random.random()*10)+2
    a=str(x)
    c=str(int(random.random()*10)+1)
    
    f.write('\\item $f(x)=x^4+'+a+'x^3+x^2+'+c+'$\n')
    f.write('\\end{enumerate}\n')
